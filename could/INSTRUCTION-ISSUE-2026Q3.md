...
## ISSUE:instruction 2026-06-08 10:00 â†’ README still stale; error codes and versioned routes undocumented; env vars incomplete
...
- Error codes (`code` field) are now returned on all error responses across 8 routes (30+ distinct values), but are documented only in `ASSET-V1.md` â€” not in the README or any API reference a developer would find
- Versioned route prefix `/1-1-1/` is still undocumented in README â€” new integrators will use legacy unversioned paths that are slated for deprecation
